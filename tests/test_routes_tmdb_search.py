...
class TestSearchTmdbRoute:
    def test_returns_200_with_results(self, client: TestClient, mocker: MockerFixture) -> None:
        mocker.patch("torrent_downloader.routers.search.search_tmdb_multi", return_value=MOCK_TMDB_RESULTS)
    ...
    def test_result_contains_expected_fields(self, client: TestClient, mocker: MockerFixture) -> None:
        mocker.patch("torrent_downloader.routers.search.search_tmdb_multi", return_value=MOCK_TMDB_RESULTS)
    ...
    def test_movie_result_fields_mapped_correctly(self, client: TestClient, mocker: MockerFixture) -> None:
        mocker.patch("torrent_downloader.routers.search.search_tmdb_multi", return_value=MOCK_TMDB_RESULTS)
        data = client.get(SEARCH_URL, params={"query": "inception"}).json()["data"]
    ...
    def test_tv_result_fields_mapped_correctly(self, client: TestClient, mocker: MockerFixture) -> None:
        mocker.patch("torrent_downloader.routers.search.search_tmdb_multi", return_value=MOCK_TMDB_RESULTS)
    ...
    def test_returns_empty_data_list_when_no_results(self, client: TestClient, mocker: MockerFixture) -> None:
