...
class TestHealthEndpointExempt:
    def test_health_reachable_without_key(
        self, unauthed_client: TestClient, mocker: MockerFixture
    ) -> None:
    ...
    def test_health_reachable_with_wrong_key(
        self, unauthed_client: TestClient, mocker: MockerFixture
    ) -> None:
        mocker.patch("torrent_downloader.routers.system.get_torrent_client", return_value=None)
        assert (
            unauthed_client.get("/api/v1/health", headers={"X-API-Key": "wrong-key"}).status_code
            == 200
        )
