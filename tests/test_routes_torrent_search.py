...
class TestSearchTorrentsRoute:
    def test_returns_503_when_client_unavailable(self, client: TestClient, mocker: MockerFixture) -> None:
    ...
    def test_returns_200_with_valid_client(self, client: TestClient, mocker: MockerFixture) -> None:
        mocker.patch("torrent_downloader.routers.search.get_torrent_client", return_value=mocker.MagicMock())
        mocker.patch("torrent_downloader.routers.search.search_torrents", return_value=MOCK_FILTERED_RESULTS)
        mocker.patch("torrent_downloader.routers.search.filter_and_sort_results", return_value=MOCK_FILTERED_RESULTS)
        response = client.get(SEARCH_URL, params={"query": "inception"})
        assert response.status_code == 200

    def test_response_shape_has_status_message_data(self, client: TestClient, mocker: MockerFixture) -> None:
        mocker.patch("torrent_downloader.routers.search.get_torrent_client", return_value=mocker.MagicMock())
        mocker.patch("torrent_downloader.routers.search.search_torrents", return_value=MOCK_FILTERED_RESULTS)
        mocker.patch("torrent_downloader.routers.search.filter_and_sort_results", return_value=MOCK_FILTERED_RESULTS)
    ...
    def test_results_grouped_by_resolution_keys(self, client: TestClient, mocker: MockerFixture) -> None:
        mocker.patch("torrent_downloader.routers.search.get_torrent_client", return_value=mocker.MagicMock())
        mocker.patch("torrent_downloader.routers.search.search_torrents", return_value=MOCK_FILTERED_RESULTS)
        mocker.patch("torrent_downloader.routers.search.filter_and_sort_results", return_value=MOCK_FILTERED_RESULTS)
    ...
    def test_returns_empty_data_when_no_results(self, client: TestClient, mocker: MockerFixture) -> None:
        mocker.patch("torrent_downloader.routers.search.get_torrent_client", return_value=mocker.MagicMock())
